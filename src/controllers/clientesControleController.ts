...
rotas.post('/find', async (req, res) => {
  const find = req.body;
  prisma.cLIENTES_CONTROLE.findMany(find)
...
rotas.get('/:id', async (req, res) => {
  const { id } = req.params
  prisma.cLIENTES_CONTROLE.findUnique({ where: { ID_CLIENTE: Number(id) } })
    .then(response => res.status(200).json(response) )
    .catch(error => res.status(200).json({result: false}));  
})
  ...
  try {
    if ( Array.isArray(data) ) {
      ...
      for (const [idx, element] of data.entries()) {
        ...
        await prisma.cLIENTES_CONTROLE.update({
            where: {
...
rotas.post('/', async (req, res) => {
  ...
  try {
    if ( Array.isArray(data) ) {
      for (const [idx, element] of data.entries()) {
         const response = await prisma.cLIENTES_CONTROLE.create({
            data: element          
        })  
